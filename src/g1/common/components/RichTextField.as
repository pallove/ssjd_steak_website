...
package g1.common.components
{
	...
	import g1.common.utils.IRTFPlugin;
	...
	public class RichTextField extends Sprite
	{
		...
		private var _keepTextFormat:Boolean;
		...
		public static const MSG_APPEND:String = "msg_append";
		...
		public function RichTextField()
		{
			...
			_textRenderer.mouseWheelEnabled = false;
			...
			this.defaultTextFormat = TextFiledConst.normal_left_fmt;
		 ...
		 */
		public function append(newText:String, newSprites:Array = null, autoWordWrap:Boolean = false, format:TextFormat = null,isNewLine:Boolean = false):int
		{		
			//append text
...
//				_textRenderer.recoverDefaultFormat();
				if (html)
				{
					...
					if (newText.indexOf("<p>") != 0 || newText.indexOf("<P>") != 0) //简单处理没有被<p>包围的文本
					{
						newText = "<p>" + newText + "</p>";
					}
					_textRenderer.htmlText += newText;	
//					_textRenderer.htmlText += "<p>" + newText + "</p>";	
					dispatchEvent(new Event(MSG_APPEND));			
				}else
				{
			...
			//var maxHeight:int = Math.max(_spriteRenderer.container.height + Math.abs(_spriteRenderer.container.x), _textRenderer.textHeight);

			//if doesn't scroll but have sprites to render, do it
			
			_textRenderer.autoSize = isNewLine?TextFieldAutoSize.LEFT:(_textRenderer.height > _height?TextFieldAutoSize.LEFT:TextFieldAutoSize.NONE);
			_textRenderer.height = isNewLine?_textRenderer.height:(_textRenderer.height > _height?_textRenderer.height:_height);
		 ...
		 * 获取应用Align之后的高度
		 */
		public function getHeight(align : String) : Number
		{
			var old : String = _textRenderer.autoSize;
			var height : Number = 0;
			_textRenderer.autoSize = align;
			height = _textRenderer.height;
			_textRenderer.autoSize = old;
			return height;
		...
		public function insertSprite(newSprite:Object, index:int = -1, autoRender:Boolean = true, cache:Boolean = false):void
		{
			...
			/*else {
				...
				fmt.size = 
				_textRenderer.setTextFormat(, index, index + 1);
			}*/
		...
		public function setKeepTextFormat(isKeepFormat:Boolean):void {
			_keepTextFormat = isKeepFormat;
		}
...
//			format.letterSpacing = width - metrics.width + 2 * _placeholderMarginH;
		...
		public function getSpriteRender():SpriteRenderer {
